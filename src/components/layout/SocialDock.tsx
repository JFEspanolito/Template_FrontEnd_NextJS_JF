...
function renderIconImage(src: string, alt: string, className: string): React.JSX.Element {
  return <Image src={src} alt={alt} className={className} width={24} height={24} />;
}

const getIcon = (iconName: string, size: 1 | 2 | 3 = 2): React.JSX.Element => {
  ...
  switch (normalizedIcon) {
    case "mail":
    case "email":
      return renderIconImage("/icons/mail.svg", "Email", iconClass);
    case "github":
      return renderIconImage("/icons/github.svg", "GitHub", iconClass);
    case "linkedin":
      return renderIconImage("/icons/linkedin.svg", "LinkedIn", iconClass);
    case "instagram":
      return renderIconImage("/icons/instagram.svg", "Instagram", iconClass);
    case "twitter":
      return renderIconImage("/icons/twitter.svg", "Twitter", iconClass);
    case "telegram":
      return renderIconImage("/icons/telegram.svg", "Telegram", iconClass);
    case "wakatime":
      return renderIconImage("/icons/wakatime.svg", "Wakatime", iconClass);
    default:
      ...
      return (
        <span
          aria-hidden="true"
          className={`${sizeClass} inline-flex items-center justify-center rounded-full border border-current text-[10px] font-semibold uppercase leading-none`}
        >
          {normalizedIcon.charAt(0) || "?"}
        </span>
      );
  }
};
    ...
    return (
      <nav aria-label="Social links" className="flex flex-col gap-2 w-full">
        {items.map((item) => (
          ...
          >
            <span className="flex-shrink-0 text-[var(--text)]">{item.icon}</span>
            {shouldShowLabel && <span className="text-sm font-medium">{item.label}</span>}
          </a>
        ))}
      </nav>
    );
    ...
    return (
      <nav aria-label="Social links" className="flex flex-wrap items-center justify-center gap-3">
        {items.map((item) => (
          ...
          >
            <span className="flex-shrink-0 text-[var(--text)]">{item.icon}</span>
            {shouldShowLabel && <span className="text-xs font-medium">{item.label}</span>}
          </a>
        ))}
      </nav>
    );
